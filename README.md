# Hi, I'm Jimena! 

**Web Developer | Cybersecurity Enthusiast | QA & Testing**

I'm a Web Development student with a growing focus on **Cybersecurity, Security Operations, and Software Quality**.

I enjoy building web applications, analyzing security alerts, working with Python, and learning how to identify and solve technical problems.

My goal is to combine **development, cybersecurity, and QA** to build more secure, reliable, and user-friendly applications.

---

## About Me

* 🎓 Studying Development and Web Design
* 🛡️ Currently building my knowledge in Cybersecurity and Blue Team practices
* 🌐 Interested in Web Development and Application Security
* 🐍 Practicing Python for automation, security, and problem solving
* 🧪 Exploring QA, software testing, and test automation
* 🔎 Interested in security alert detection, log analysis, and incident investigation
* 📚 Always learning and building hands-on projects

---

## Cybersecurity

I'm currently developing practical skills in:

* 🔎 Security Alert Analysis
* 📊 Log Analysis
* 🚨 Incident Detection & Investigation
* 🧪 Vulnerability Assessment
* 🌐 Web & Application Security
* 🔐 Security Fundamentals
* 🖥️ Linux Fundamentals
* 📋 Security Documentation & Reporting

---

## Web Development

### Technologies & Tools

* HTML5
* CSS3
* JavaScript
* Bootstrap
* WordPress
* Figma
* Responsive Web Design
* UX/UI Design
* REST APIs
* Postman

I'm particularly interested in building applications while considering **security, accessibility, usability, and performance**.

---

## Python

Currently using Python to improve my:

* Programming fundamentals
* Automation skills
* Data and log processing
* Problem solving
* Security-related scripting

Some of my upcoming projects will focus on using Python for **security analysis and automation**.

---

## QA & Testing

I'm developing skills in:

* Test Cases
* Functional Testing
* Regression Testing
* Bug Reporting
* API Testing
* Test Documentation
* Web Application Testing
* Test Automation

I'm especially interested in combining **QA and cybersecurity** to identify vulnerabilities and improve application reliability.

---

## Currently Learning

```text
🛡️ Cybersecurity & Blue Team
🐍 Python
🐧 Linux
🧪 QA & Test Automation
🔎 Security Alert & Log Analysis
🌐 Application Security
🗄️ SQL & Databases
☁️ Cloud Fundamentals
```

---

## 🚀 Featured Projects

### 🌎 Full-Stack Travel Management System

A full-stack web application for managing travel tours, users, and reservations, featuring a RESTful API, MongoDB database, authentication, and a responsive web interface.

**Focus:** Full-Stack Development · REST API · API Testing · Authentication · Database Management

**Tech Stack:** HTML · CSS · JavaScript · Bootstrap · Node.js · Express · MongoDB · Mongoose · JWT · Postman

---

### 🧪 Web Application QA Testing

> Testing a web application through test cases, bug reports, API testing, and regression testing.

**Focus:** QA · Testing · Postman · Web Development

---

### 🌐 Cybersecurity Dashboard

> A web-based dashboard for visualizing security events and basic security metrics.

**Focus:** HTML · CSS · JavaScript · Data Visualization

---

## 📫 Contact

I'm always interested in connecting with people in **Web Development, Cybersecurity, QA, and Technology**.

* 💼 LinkedIn: https://www.linkedin.com/in/jimena-p/
* 📧 Email: jimena.prado.arce@gmail.com
* 💻 GitHub: https://github.com/JimenaPrado

---

⭐ *I'm currently building my portfolio through hands-on projects and continuous learning.*
